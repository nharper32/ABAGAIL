...
public class CountOnesRandOpt {
  ...
  public static void CountOnes(int N) {
    ...
    System.out.println("COUNT ONES OPTIMZATION PROBLEM:");
    ...
    EvaluationFunction eval_func = new CountOnesEvaluationFunction();
    ...
    int[] numIterationsRHC = {1, 10, 20, 30, 40, 50, 100, 200, 500, 700, 1000, 2000, 5000, 10000};

    for (int iterations: numIterationsRHC) {

      RandomizedHillClimbing rand_hill = new RandomizedHillClimbing(hill_climbing);
      fit = new FixedIterationTrainer(rand_hill, iterations);
      fit.train();
    ...
    int[] numIterations = {1, 10, 20, 30, 40, 50, 100, 200, 500, 700, 1000, 2000, 5000, 10000};
    ...
    for (int i=0; i < 3; i++) {
      ...
      for (double temp: temps) {
        ...
        SimulatedAnnealing sim_anneal = new SimulatedAnnealing(temp, .99, hill_climbing);
        fit = new FixedIterationTrainer(sim_anneal, 1500);
        fit.train();
    ...
    for (int i=0; i < 3; i++) {
      ...
      for (int pops: popSizes) {

        StandardGeneticAlgorithm stand_gen =new StandardGeneticAlgorithm(pops, pops/2, pops/20, gen_alg);
        fit = new FixedIterationTrainer(stand_gen, 700);
        fit.train();
    ...
    int[] samples = {10, 20, 40, 50, 100, 150, 200, 250, 300, 400, 500, 750, 1000, 10000};
    ...
    for (int sample: samples) {
      ...
      MIMIC mimic = new MIMIC(sample, sample/10, prob_opt);
      fit = new FixedIterationTrainer(mimic, 30);
      fit.train();
